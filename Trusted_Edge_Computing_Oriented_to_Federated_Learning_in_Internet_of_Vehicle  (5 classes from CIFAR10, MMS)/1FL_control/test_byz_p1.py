...
def parse_args():
    ...
    parser.add_argument("--trust_value1", help="trust values for weighted average", type=float, default=0)
    parser.add_argument("--trust_value2", help="trust values for weighted average", type=float, default=0)
    parser.add_argument("--trust_value3", help="trust values for weighted average", type=float, default=0)
    parser.add_argument("--trust_value4", help="trust values for weighted average", type=float, default=0)
    parser.add_argument("--trust_value5", help="trust values for weighted average", type=float, default=0)
    parser.add_argument("--trust_value6", help="trust values for weighted average", type=float, default=0)
    parser.add_argument("--trust_value7", help="trust values for weighted average", type=float, default=0)
    parser.add_argument("--trust_value8", help="trust values for weighted average", type=float, default=0)
    parser.add_argument("--trust_value9", help="trust values for weighted average", type=float, default=0)
    parser.add_argument("--trust_value10", help="trust values for weighted average", type=float, default=0)
...
#
   ...
   #     allclasses_index = np.sort(allclasses_index)
   #     print("allclasses_index_aftersort {}, and allclasses_indexLEN {}, and indextype {}".format(allclasses_index,len(allclasses_index),type(allclasses_index)))
   #     train_dataset_5classes = np.array(train_dataset)[allclasses_index]
   #     print("5classes type {}, and 5classesLEN {}".format(type(train_dataset_5classes),train_dataset_5classes.shape))
    #    print("5classes has len {}, and 5classes[0] has len {}, 5classestype {}, 5classes[0]type {}, 5classes[0][0].shape {}, 5classes[0][1]type {}".format(len(train_dataset_5classes),len(train_dataset_5classes[0]),type(train_dataset_5classes),type(train_dataset_5classes[0]),train_dataset_5classes[0][0].shape,type(train_dataset_5classes[0][1])))
    #    print("train_dataset[0][0] {}, and train_dataset[0][1] {}, and train_dataset[0][0].shape {}".format(train_dataset_5classes[0][0],train_dataset_5classes[0][1],train_dataset_5classes[0][0].shape))
...
#choose 5 classes from CIFAR10:labels=1,4,5,7,9
#        for X_batch,y_batch in train_data:
#            print("X_batch_train has shape[0] {}, and X_batch_train has shape {}".format(X_batch.shape[0],X_batch[0].shape))
#            print("y_batch_train[0:5] {}, and y_batch[3] is  {}, and y_batch[4] is  {}, and y_batch[7] is  {}".format(y_batch[0:5],y_batch[3],y_batch[4],y_batch[7]))
#            class_1_index = np.where(y_batch == 1)[0]
#            print("class_1_index {}, and class_1_indexLEN {}".format(class_1_index,len(class_1_index)))
#            class_4_index = np.where(y_batch == 4)[0]
#            class_5_index = np.where(y_batch == 5)[0]
#            class_7_index = np.where(y_batch == 7)[0]
#            class_9_index = np.where(y_batch == 9)[0]
#            allclasses_index = np.concatenate((class_1_index,class_4_index,class_5_index,class_7_index,class_9_index))
#            print("allclasses_index_beforesort {}, and allclasses_indexLEN {}".format(allclasses_index,len(allclasses_index)))
#            allclasses_index = np.sort(allclasses_index)
#            print("allclasses_index_aftersort {}, and allclasses_indexLEN {}".format(allclasses_index,len(allclasses_index)))
#            X_batch = X_batch[allclasses_index]
#            y_batch = y_batch[allclasses_index]
#            print("X_batch_train_NEW has shape {}, and y_batch_train_NEW has shape {}".format(X_batch.shape,y_batch.shape))
#            print("y_batch[0:5] {},y_batchLEN {}".format(y_batch[0:100],len(y_batch))) #25000 caidui
...
def assign_data(train_data, bias, ctx, num_labels=5, num_workers=10, server_pc=10, p=0.1, dataset="CIFAR10", seed=1):
...
def main(args):
    ...
    niter = args.niter
    frac = args.frac
    trust_value_list = [args.trust_value1,args.trust_value2,args.trust_value3,args.trust_value4,args.trust_value5,args.trust_value6,args.trust_value7,args.trust_value8,args.trust_value9,args.trust_value10]
    value_remove = min(trust_value_list)
#new list containing 9 clients
    trust_value_list.remove(value_remove)
    print("trust_value_list",trust_value_list)
    ...
    with ctx:
        ...
        wandb.init(
            project="FL_application_control(MMS)",
            name="FL_app_controlGroup(MMS)_5classes" + str(args.dataset) + "+" + "net " + str(args.net) + "+" +"niter " + str(args.niter) +"+nworkers " + str(
...
########shiyishi tiqu FEI
        #X_trainTest,y_trainTest = list(*train_data)
        #print("X_batch_train has shape {}, and y_batch_train has shape {}".format(X_trainTest.shape,y_trainTest.shape))
#########
        for X_batch,y_batch in train_data:
...
# succeed! Extracting classes from train_data.
     #   for X_batch,y_batch in train_data:
     #       print("X_batch_train has shape[0] {}, and X_batch_train has shape {}".format(X_batch.shape[0],X_batch[0].shape))
     #       print("y_batch_train[0:5], and y_batch[41] is {} {}".format(y_batch[0:5],y_batch[41]))
     #       class_0_indexall = np.where(y_batch == 0) #32 ge array, zhiyou 1st you yong.?
     #       class_0_index=class_0_indexall[0] #tiqu real 0_index.
     #       print("class_0_index {}, and class_0_indexLEN {}".format(class_0_index,len(class_0_index)))
     #       y_train_class_0 = y_batch[class_0_index]
            #print("X_train_class_1 has shape {}, and X_train_class_1 has shape {}".format(X_train_class_1.shape,y_train_class_1.shape))
     #       print("y_train_class_0[0:5] {}".format(y_train_class_0[0:5]))
     #       print("y_train_class_0LEN {}".format(len(y_train_class_0))) #5000 caidui
            #X_batch = np.concatenate((X_train_class_1,X_train_class_4,X_train_class_5,X_train_class_7,X_train_class_9))
            #y_batch = np.concatenate((y_train_class_1,y_train_class_4,y_train_class_5,y_train_class_7,y_train_class_9))
     #       print("X_batch_train_NEW has shape {}, and y_batch_train_NEW has shape {}".format(X_batch.shape,y_batch.shape))

#plot pics
   #     for X_batch_train,y_batch_train in train_data:
   #         plot_sample(X_batch_train,y_batch_train,0)
   #         plot_sample(X_batch_train,y_batch_train,1)
   #         plot_sample(X_batch_train,y_batch_train,2)
   #         plot_sample(X_batch_train,y_batch_train,3)
   #         plot_sample(X_batch_train,y_batch_train,4)
   #         plot_sample(X_batch_train,y_batch_train,5)
   #         plot_sample(X_batch_train,y_batch_train,6)
   #         plot_sample(X_batch_train,y_batch_train,7)
   #         plot_sample(X_batch_train,y_batch_train,-1)
    #        plot_sample(X_batch_train,y_batch_train,-2)
   #         plot_sample(X_batch_train,y_batch_train,-3)
   #         plot_sample(X_batch_train,y_batch_train,-4)
   ...
   #         plot_sample(X_batch_test,y_batch_test,0)
   #         plot_sample(X_batch_test,y_batch_test,1)
   #         plot_sample(X_batch_test,y_batch_test,2)
 #           plot_sample(X_batch_test,y_batch_test,3)
  #          plot_sample(X_batch_test,y_batch_test,4)
   #         plot_sample(X_batch_test,y_batch_test,5)
    #        plot_sample(X_batch_test,y_batch_test,6)
     #       plot_sample(X_batch_test,y_batch_test,7)
      #      plot_sample(X_batch_test,y_batch_test,-1)
       #     plot_sample(X_batch_test,y_batch_test,-2)
        #    plot_sample(X_batch_test,y_batch_test,-3)
         #   plot_sample(X_batch_test,y_batch_test,-4)
        ...
        for e in range(niter):
            ...
            train_locals_loss = []
            print("m:",m)
            idxs_minigroup = np.random.choice(range(num_workers-1), size=m, replace=False) 
            for i in idxs_minigroup:
                ...
                score_list.append(trust_value_list[i])
...
#                grad_list.append([param.grad().copy() for param in net.collect_params().values()])
                ...
                nd_aggregation1.fltrust(grad_list, net, lr, args.nbyz, byz, score_list,value_remove)
